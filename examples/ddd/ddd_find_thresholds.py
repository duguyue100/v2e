...
from v2e.v2e_utils import DVS_HEIGHT
...
if __name__ == "__main__":
    ...
    if args.y is None:
        args.y = tuple(0, DVS_HEIGHT)  # type: ignore
    ...
    if Path(slomoVideoFile).exists():
        ...
        slomoVideoFile = None  # type: ignore
    ...
    dvsOnCount = np.count_nonzero(dvsEvents[:, 3] == 1)  # type: ignore
    dvsOffCount = dvsEvents.shape[0] - dvsOnCount  # type: ignore

    with TemporaryDirectory() as interp_frames_dir:
        logger.info("intepolated frames folder: %s", interp_frames_dir)
        slomo = SuperSloMo(
        ...
        slomo.interpolate(
            images=frames["frame"],  # type: ignore
            output_folder=interp_frames_dir,
        )  # writes all frames to interp_frames_folder
        frame_ts = slomo.get_interpolated_timestamps(frames["ts"])  # type: ignore
        height, width = frames["frame"].shape[1:]  # type: ignore
        nFrames = frames.shape[0]  # type: ignore
        ...
        for threshold in tqdm(thresholds, desc="thr sweep"):
            ...
            for i in range(nFrames):
                events_v2e = emulator.generate_events(frames["frame"][i], frame_ts[i])  # type: ignore
    ...
    if pos_thres > 0 and neg_thres > 0:
        logger.info("Optimal Pos Threshold Found: %s", pos_thres)
        logger.info("Optimal Neg Threshold Found: %s", neg_thres)
