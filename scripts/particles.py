...
class particles(
    ...
    def __init__(
        ...
        parser.add_argument(
            "--num_particles", type=int, default=particles.NUM_PARTICLES
        )
        ...
        self.contrast: float = (
            args.contrast
        )  # compare this with pos_thres and neg_thres and sigma_thr, e.g. use 1.2 for dot to be 20% brighter than backgreound
    ...
    def cleanup(self):
        logger.info(
            f"particles() generated {self.particle_count} particles in {self.time}s"
        )
    ...
    def next_frame(self) -> Tuple[Optional[np.ndarray], float]:
        ...
        if self.preview and self.frame_number % 50 == 0:
            ...
            if k == ord("x"):
                logger.warning(
                    "aborted output after {} frames".format(self.frame_number)
                )
                cv2.destroyAllWindows()
                return None, time
        self.frame_number += 1
