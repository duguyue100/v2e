...
class particles(SyntheticInput): # the class name should be the same as the filename, like in Java  # type: ignore
    ...
    def __init__(self, width: int = 346, height: int = 260, avi_path: str | None = None, preview=False,  # type: ignore
        ...
        self.times: "Any" = np.arange(0, self.t_total, self.dt) # note floating point roundoff can produce small eps on top of step  # type: ignore
        ...
        self.pix_arr: np.ndarray = self.bg * np.ones((self.h, self.w), dtype=np.float32)  # type: ignore
    ...
    class particle:
        def __init__(self,  outer, width:int, height:int , time:float, radius:float, speed_min, speed_max, start_on_edge:bool=False):  # type: ignore
            self.width=width
            self.height=height
            if start_on_edge:
                ...
                angle_rad=0
                if edge==1: #n
                    angle_rad=np.random.uniform(np.pi/4,-.75*np.pi)  # type: ignore
                elif edge==0: # s
                    angle_rad=np.random.uniform(np.pi/4,.75*np.pi)  # type: ignore
                elif edge==3: # e
                    angle_rad=np.random.uniform(-np.pi/4,np.pi/4)  # type: ignore
                elif edge==2: # w
                    angle_rad=np.random.uniform(np.pi/4,3*np.pi/2-np.pi/4)  # type: ignore
            else:
                ...
                pos_x=np.random.uniform(0,width)  # type: ignore
                pos_y=np.random.uniform(0,height)  # type: ignore
                # pos_x=np.random.uniform(0,5) # to debug single pixel generate most particle near corner
                # pos_y=np.random.uniform(0,5)
            angle_rad=np.random.uniform(0, 2*np.pi)  # type: ignore
...
@njit  # type: ignore
def fill_dot(pix_arr: np.ndarray, x: float, y: float, fg: float, bg: float, radius: float):  # type: ignore
...
if __name__ == "__main__":
    ...
    with tqdm(total=m.total_frames(), desc='moving-dot', unit='fr') as pbar:  # instantiate progress bar  # type: ignore
