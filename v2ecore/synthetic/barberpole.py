...
class barberpole(SyntheticInput): # the class name should be the same as the filename, like in Java  # type: ignore
    ...
    def __init__(self, width: int = 346, height: int = 260, avi_path: str | None = None, preview=False,  # type: ignore
        ...
        self.bg=SyntheticInput.BACKGROUND #127  # type: ignore
    ...
    def next_frame(self) -> tuple[np.ndarray | None, float]:  # type: ignore
        ...
        self.pix_arr=self.bb_func(self.y[:,None],self.x[None,:],  time)  # type: ignore
        ...
        if self.avi_path is not None:
            self.out.write(cv2.cvtColor(self.pix_arr, cv2.COLOR_GRAY2BGR))  # type: ignore
    ...
    def bb_func(self, y,x, t):  # type: ignore
