...
class SyntheticInput: # the class name should be the same as the filename, like in Java
    ...
    def __init__(self, width: int = 346, height: int = 260, avi_path: str | None = None, preview=True, args:list | None=None, parent_args:argparse.Namespace | None=None) -> None:  # type: ignore
        ...
        self.pix_arr: np.ndarray = self.bg * np.ones((self.height, self.width), dtype=np.uint8)  # type: ignore
        self.width = width
        self.height = height
        self.frame_number=0
        self.args=args
...
if __name__ == "__main__":
    ...
    with tqdm(total=m.total_frames(), desc='synthetic_input', unit='fr') as pbar:  # instantiate progress bar  # type: ignore
