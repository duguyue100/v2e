...
class DVSTextOutput:
    """outputs text format DVS events to file according to events.txt format in
    ...
    def close(self):
        if self.file:
            logger.info(
                "Closing {} after writing {} events".format(
                    self.filepath, EngNumber(self.numEventsWritten)
                )
            )
            self.file.close()
            self.file = None
