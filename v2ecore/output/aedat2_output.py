...
class AEDat2Output:
    ...
    def __init__(self, filepath: str, output_width=346, output_height=240):
        ...
        if output_width == 346 and output_height == 260:
            ...
            self.xShiftBits = 12
            self.polShiftBits = (
                11  # see https://inivation.com/support/software/fileformat/#aedat-20
            )
            ...
            raise ValueError(
                f"AEDAT-2.0 output width={output_width} height={output_height} not supported; add your camera to {__name__} or use one of the predefined DVS cameras, e.g. --dvs346 or --dvs240 that have sizes {self.SUPPORTED_SIZES}"
            )
        ...
        try:
            ...
            logger.error(
                "caught {}:\n  could not open {} for writing; maybe jAER has it open?".format(
                    err, filepath
                )
            )
            v2e_quit(1)
    ...
    def close(self):
        if self.file:
            logger.info(
                "Closing {} after writing {} events ({} on, {} off)".format(
                    self.filepath,
