...
def compute_event_map(
    diff_frame: Any, pos_thres: Any, neg_thres: Any
...
def compute_photoreceptor_noise_voltage(
    ...
    if rate_per_bw > 0.5:
        logger.warning("shot noise rate per hz of bandwidth is larger than 0.1")
    x = math.log10(rate_per_bw)
    if x < -5.0:
        logger.warning("desired noise rate is too low")
    elif x > 0.0:
        logger.warning("desired noise rate is too large")
...
def generate_shot_noise(
    ...
    neg_thres_pre_prob: Any,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Generate shot noise."""
    if shot_noise_rate_hz * delta_time > 1:
        logger.warning(
            "shot_noise_rate_hz*delta_time=%s*%s=%s is too large",
            shot_noise_rate_hz,
            delta_time,
            shot_noise_rate_hz * delta_time,
        )
