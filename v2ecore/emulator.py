...
class EventEmulator:
    ...
    # scidvs adaptation
    def scidvs_dvdt(
        self, v: torch.Tensor, tau: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
    ...
    def __init__(
        ...
        self.photoreceptor_noise_arr: Optional[Any] = (
        ...
        if self.record_single_pixel_states is None:
            ...
            self.single_pixel_states = {
                ...
                "final_neg_evts_frame": np.empty(self.SINGLE_PIXEL_MAX_SAMPLES)
                * np.nan,
                "final_pos_evts_frame": np.empty(self.SINGLE_PIXEL_MAX_SAMPLES)
                * np.nan,
    ...
    def reset(self) -> None:
        ...
        self.new_frame: Optional[torch.Tensor] = (
        ...
        self.base_log_frame: Optional[torch.Tensor] = (
    ...
    def _show(self, inp: torch.Tensor, name: str) -> None:
        ...
        cv2.putText(
            ...
            org=(0, int(self.output_height or 0)),
            fontScale=1.3,
            color=(0, 0, 0),
        ...
        cv2.putText(
            img,
            f"fr:{self.frame_counter} t:{self.t_previous:.4f}s",
            org=(1, int(self.output_height or 0) - 1),
            fontScale=1.3,
            color=(255, 255, 255),
            fontFace=cv2.FONT_HERSHEY_PLAIN,
            thickness=1,
    ...
    def generate_events(self, new_frame: Any, t_frame: float) -> Optional[Any]:
        ...
        max_num_events_any_pixel = (
            max_num_events_any_pixel.cpu().numpy().item()
        ...
        if self.record_single_pixel_states is not None:
            if (
                self.single_pixel_states is not None
                and self.single_pixel_sample_count < self.SINGLE_PIXEL_MAX_SAMPLES
            ):
                ...
                self.single_pixel_states["final_neg_evts_frame"][k] = (
                    final_neg_evts_frame[self.record_single_pixel_states]
                )
                self.single_pixel_states["final_pos_evts_frame"][k] = (
                    final_pos_evts_frame[self.record_single_pixel_states]
                )
    ...
    def get_event_list_from_coords(
        self,
        pos_event_xy: Tuple[torch.Tensor, ...],
        neg_event_xy: Tuple[torch.Tensor, ...],
        ts: torch.Tensor,
        ...
        if num_events > 0:
            ...
            events_curr_iter[
                num_pos_events:, 3
            ] = -1  # neg events polarity is -1 so flip the signs
