...
if __name__ == "__main__":
    ...
    logger.info(
        "Input video %s: width: %s \t height: %s", dvs_video_real_avi, width, height
    )
    ...
    stop = args.stop + ts0
    i = 0
    logger.info("writing video output file %s with ROI labeled", outputVideoPath)
    while cap.isOpened():
        ret, img = cap.read()
        ...
        i += 1
    out.release()
    logger.info("finished writing %s.", outputVideoPath)
