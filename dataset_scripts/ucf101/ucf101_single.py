"""TODO not correct doc Convert videos in UCF-101 dataset into event frames. In each
action class, one video is randomly selected.
    ...
    with TemporaryDirectory() as dirname:
        ...
        for factor in [1, args.sf]:
            ...
            r_slomo = VideoSequenceFiles2EventsRenderer(
                ...
                args.neg_thres,
                os.path.join(
                    args.output_dir, "interpolated_{:d}.avi".format(int(factor * fps))
                ),
            )
            ...
            r_input = ImageSequenceArray2EventsRenderer(
                ...
                args.neg_thres,
                os.path.join(
                    args.output_dir, "input_{:d}.avi".format(int(factor * fps))
                ),
            )
