...
if __name__ == "__main__":
    ...
    if not Path(dvs_v2e_npy).exists() or not Path(dvs_real_npy).exists():
        logger.error(
            "numpy event files {} or {} not accessible\nDid you run ddd-v2e.py first?".format(
                dvs_v2e_npy, dvs_real_npy
            )
        )
        sys.exit(1)
    ...
    logger.info(
        "Input video {}: width: {} \t height: {}".format(
            dvs_video_real_avi, width, height
        )
    )
