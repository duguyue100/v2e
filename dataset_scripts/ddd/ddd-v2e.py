...
if __name__ == "__main__":
    ...
    if f:
        logger.error(
            "output folder {} already exists\n it holds files {}\n - use --overwrite".format(
                os.path.abspath(output_folder), f
            )
        )
        quit()
    ...
    if not exposure_mode == ExposureMode.DURATION:
        raise ValueError(
            "only dvs_exposure=duration is currently supported (mode {} not allowed)".format(
                exposure_mode
            )
        )
    ...
    for i in tqdm(range(startPacket, stopPacket), desc="v2e-ddd20", unit="packet"):
        ...
        if numpy_output and platform.startswith("linux"):
            ...
            if usageRSS_kB > memoryLimit / 1024:
                logger.warning(
                    "\n*** memory usage (RSS) is {}B".format(
                        EngNumber(1024 * usageRSS_kB)
                    )
                )
                memoryLimit += 1e9
    ...
    if output_folder and numpy_output:
        ...
        logger.info(
            "saved numpy files with real and v2e events to {}".format(output_folder)
        )
    ...
    logger.info(
        "done processing {} frames in {}s ({})\n see output folder {}".format(
            num_frames, EngNumber(totalTime), throughputStr, output_folder
        )
    )
    logger.info(
    ...
    logger.info(
        "avg event rate {}Hz ({}Hz on, {}Hz off)".format(
